...
def eval(args, model, device, loader, evaluation_mode):
    ...
    with torch.no_grad():
        ...
        if args.output_model_file is not None:
            filename = '{}/{}/{}'.format(args.output_model_file, args.seed, evaluation_mode)
            print('filename\t', filename)
            np.savez(filename, y_true_list=y_true_list, y_pred_list=y_pred_list, id_list=id_list)

            data = np.load('{}.npz'.format(filename))
            a = data['y_true_list']
            b = data['y_pred_list']
            c = data['id_list']
...
def save_model(save_best=False):
    if not args.output_model_file == '':
        ...
        if save_best:
            ...
            torch.save(saver, args.output_model_file + '.pth')
    return
...
if __name__ == '__main__':
    ...
    for seed in range(5):
        ...
        test_results = np.load('gnn/{}/{}/{}.npz'.format(args.dataset, args.seed, "model_test"))
    ...
    print('========== Summary ==========')
    print('Train ROC:\t{} -/+ {}'.format(np.mean(train_roc_list), np.std(train_roc_list)))
    print('Valid ROC:\t{} -/+ {}'.format(np.mean(valid_roc_list), np.std(valid_roc_list)))
    print('Test ROC:\t{} -/+ {}'.format(np.mean(test_roc_list), np.std(test_roc_list)))
    print('\n\n\n\n')
